...
for folder_name in os.listdir(A1_PATH):
    ...
    for i in range(len(df)):
        ...
        # cut normal clip if first time_start != 0
        # time_start = sum([a * b for a, b in zip(ftr, map(int, start_time_lst[i].split(':')))])
        # if (i == 0 and time_start != 0) or (filename_lst[i].split('_')[0] != filename_lst[i - 1].split('_')[0]):
        #     clip_path = '{}/{}/{}_{}_{}.MP4'.format(A1_CLIP_PATH, 0, file_name, 0, time_start)
        # if not os.path.exists(clip_path):
        #     print("Process {}".format(clip_path))
        #     cut_video(clip, 0, time_start, clip_path)
        # else:
        #     print("Already process {}".format(clip_path))

        # cut normal clip if second time_start != 0
        # time_start = sum([a * b for a, b in zip(ftr, map(int, end_time_lst[i].split(':')))])
        # time_end = sum([a * b for a, b in zip(ftr, map(int, start_time_lst[i + 1].split(':')))])
        # if (time_start > time_end and time_end != 0):
        #     clip_path = '{}/{}/{}_{}_{}.MP4'.format(A1_CLIP_PATH, 0, file_name, 0, time_end)
        # if not os.path.exists(clip_path):
        #     print("Process {}".format(clip_path))
        #     cut_video(clip, 0, time_end, clip_path)
        # else:
        #     print("Already process {}".format(clip_path))
        print(file_name)