...
def activity_localization(prob_sq):
    ...
    threshold = np.mean(action_probs) # lambda 1
...
def merge_and_remove(data):
    ...
    for i in range(1, 3):
        ...
        for label in list_label:
            ...
            for j in range(len(data_video_label) - 1):
                if data_video_label.loc[j + 1, 2] - data_video_label.loc[j, 3] <= 16: # lambda 2, merge two clips with same class
    ...
    df_total = df_total[df_total[3] - df_total[2] > 6] # lambda 3, remove action segments
...
def general_submission(data):
    ...
    merge_and_remove(data_filtered) # second method of post-processing
...
if __name__ == "__main__":
    ...
    for i in range(1, len(vid_info) + 1): # 11
        ...
        prediction_smoothed = smoothing(prob_actions, 3) # first method of post-processing
